...
class Articulation(BaseArticulation):
    ...
    def _rebind_after_physics_ready(self) -> None:
        ...
        configs wrote is silently discarded: measured on a humanoid, stiffness 4400 -> 53026,
        damping 40 -> 2148, armature 0.1 -> 0.0 and the effort limit -> ``inf``. Those values are
        outside what MJWarp can integrate, so the first commanded motion after the reset drives the
        whole articulation non-finite, while a zero command still looks healthy. Re-applying the
        actuator configs restores the task's gains on the rebuilt model.
        """
