...
def _mpm_manager():
    ...
    Importing it pulls in ``warp.fem``, which resolves to the Warp bundled with Kit's
    ``omni.warp.core`` extension while ``warp._src`` resolves to the installed one, so the import
    raises in any app that loads that extension. A coupled scene with no MPM entry must not
    trigger it.
    """
